...
public class RestFeatureManager extends FeatureStoreBaseRestHandler {
  ...
  RestChannelConsumer delete(
    ...
    return (channel) -> {
      // RestToXContentListener<DeleteResponse> restR = new RestToXContentListener<>(channel, (r) ->
      // r.getLocation(routing));
      RestToXContentListener<DeleteResponse> restR =
          (new RestToXContentListener<>(channel, (r) -> r.status()));
      client
  ...
  RestChannelConsumer get(NodeClient client, String type, String indexName, RestRequest request) {
    ...
    return (channel) ->
        client
            ...
            .setRouting(routing)
            .execute(
                new RestToXContentListener<GetResponse>(
                    channel, r -> r.isExists() ? OK : NOT_FOUND));
  }
